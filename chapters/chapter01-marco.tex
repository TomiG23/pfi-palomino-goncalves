...
Complementa los datos transaccionales con el comportamiento histórico de la base de compradores. Se utilizan \texttt{total\_spent} y \texttt{orders\_count} para calcular el valor de vida del cliente (CLV) y segmentar la base según su nivel de actividad, y \texttt{last\_order\_date} para identificar clientes en riesgo de abandono y fundamentar recomendaciones de retención o reactivación.
...
La segunda capa consiste en datos sintéticos generados programáticamente a partir del esquema oficial de la API de Tiendanube. Dado que la tienda demo no cuenta con historial transaccional acumulado, se generarán conjuntos de datos ficticios pero estructuralmente fieles a los contratos de la API, cubriendo los tres rubros objetivo del proyecto: gastronomía, indumentaria y electrónica. Estos datos permitirán simular patrones de comportamiento relevantes para el motor de análisis, tales como estacionalidad, rotación de stock y frecuencia de recompra, en volúmenes suficientes para evaluar la calidad de las recomendaciones generadas.
